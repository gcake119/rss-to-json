import { paginate, renderPager, fmtDate, htmlEscape } from './util.js';
const PAGESIZE = 12;
...
function getEpisodeImage(ep, fallback) {
    ...
    if (ep["itunes:image"]) {
      ...
      if (ep["itunes:image"].href && typeof ep["itunes:image"].href === "string")
        return ep["itunes:image"].href;
      // 或字串
      if (typeof ep["itunes:image"] === "string")
        return ep["itunes:image"];
    }
    // fallback: 節目封面
...
export async function renderPodcastList(jsonPath, page = 1) {
  ...
  const showCover = items?.[0]?.coverimg || items?.[0]?.image || items?.[0]?.showCover || '/img/podcast-default.png';
  ...
  pageItems.forEach(ep => {
    ...
    html += `
      <div class="card" style="cursor:pointer;" onclick="location.hash='#viewPodcast?json=${encodeURIComponent(jsonPath)}&guid=${encodeURIComponent(ep.guid)}'">
        <div style="display:flex;align-items:center;">
          ${imgSrc ? `<img src="${imgSrc}" alt="" style="width:80px;height:80px;border-radius:8px;object-fit:cover;margin-right:1rem;">` : ''}
          <div>
...
export async function renderPodcastDetail(jsonPath, guid) {
  ...
  let description = ep.description_html
    || ep.description
    || ep.content
    || '';
